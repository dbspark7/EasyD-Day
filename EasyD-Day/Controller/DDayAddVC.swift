...
class DDayAddVC: UITableViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        // 이벤트 기록
        Analytics.logEvent("추가_화면", parameters: ["추가_화면": "추가_화면" as NSObject])
...
let info = convertFromUIImagePickerControllerInfoKeyDictionary(info)
    ...
    private func setup() {
        ...
        if self.param != nil {
            ...
            self.ddayDate.textColor = UIColor.black
            ...
            if param?.ddayTitle == "" {
                ...
                self.ddayTitle.textColor = UIColor.black
            }
            ...
            if param?.notificationTime != nil && (param?.notificationTime?.timeIntervalSinceNow)! > 0 {
                ...
                self.notificationTime.textColor = UIColor.black
                self.inputNotificationTime = param?.notificationTime
            }
            
            // 디데이 이미지
            if param?.imageExistance == true {
                self.ddayImage.image = UIImage(data: self.ud.data(forKey: (param?.identifier)!)!)
            }
        }
    }
    ...
    private func setDDayDate() {
        ...
        let dayAlert = CustomAlertController(title: "D-Day 날짜 선택", message: nil, preferredStyle: .alert)
    ...
    private func setDDayTitle() {
        let alert = CustomAlertController(title: "제목", message: nil, preferredStyle: .alert)
    ...
    private func setNotificationTime() {
        ...
        let timeAlert = CustomAlertController(title: "D-Day 알림 시각", message: nil, preferredStyle: .alert)
    ...
    @IBAction func save(_ sender: Any) {
        ...
        if param?.imageExistance == true {
            self.ud.removeObject(forKey: (param?.identifier)!)
            self.ud.synchronize()
        }
        ...
        // Today Extension 설정
        if self.dao.fetch().count == 0 {
            data.isTodayExtensionSelected = true
        } else {
            data.isTodayExtensionSelected = false
        }
